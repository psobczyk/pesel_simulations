...
SNRs <- c(0.25, 0.375, 0.5, 0.625, 0.75, 0.875, 1)
vars <- c(50, 2000)
ns <- c(2000, 50)
...
# filename <- paste0("fixed_effects_simulations_big_n_", args[[1]], "_", gsub("-", "_", Sys.Date()), ".Rdata")
# save.image(filename)
...
selected.snrs <- c(0.250, 0.375, 0.500, 0.625, 0.750, 0.875, 1)
selected.variables.number <- c(50, 2000)
...
y %>% filter(method %in% selected.method2,
  ...
  mutate(snr=factor(snr), scenario=ifelse(variables==50, "n=2000, p=50", "n=50, p=2000")) %>% 
  group_by(snr, scenario, method) %>% 
...
ggplot(data5, aes(x=snr, y=MeanPCs)) + 
  ...
  geom_linerange(aes(ymin=MeanPCs-SdPCs, ymax=MeanPCs+SdPCs, color=method, alpha=1)) +
  facet_grid(.~scenario) +
...
ggsave(filename = "figure2.png", height = 8, width = 14)